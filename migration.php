...
if(is_array($responsebids) && !empty($responsebids)){
    ...
    for($j=0;$j<count($responsebids);$j++)
    {
        ...
        if(count($DATA1))
        {
			...
			OA_Dal_Delivery_query("UPDATE `rv_dj_ba_statss` SET response_count='".$tot_count1."' WHERE id='".$DATA1['id']."'");
			$updated++;
		...
		else
		{
			...
			echo  $sql1="INSERT INTO `rv_dj_ba_statss`(`campaignid`,`bannerid`, `country`, `country_name`, `os`, `ip`, `user_agent`, `device_type`, `device_make`, `device_model`, `response_count`,mediatype , `datatime`) VALUES ($creativeId,$bannerId,$country,$country,$os,$ip,$user_agent,$device_type,$device_make,$device_model,$count,'$mediatype','$start')";
